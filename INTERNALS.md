...
| TS 同期 | 払い出しチャンク先頭の同期バイト 一致率 100% (427/427, 480/480, 1204/1204) |
...
| | `SetChannel()` | 映像が出るまで |
| --- | --- | --- |
| 地デジ / BS / CS 初回 | 約 1.5 秒 | 約 3.3 秒 |
| 地デジ / BS / CS チャンネル変更 | 30ms〜1.0 秒 | 約 1.9 秒 |
| BS4K 初回 | 約 1.5 秒 | 約 30 秒 |
| BS4K チャンネル変更 | 約 1.5 秒 | 約 9.4 秒 |

`SetChannel()` が一律 1.5 秒前後で済んでいるのは `EARLY_RESPONSE=1` のためです。0 にすると
「映像が出るまで」とほぼ同じ時間だけ `SetChannel()` がブロックします(BS4K は実測 29.5 秒)。

初回の 1.5 秒は、`localhost` が先に `::1` へ解決されるのに EDCB の HTTP サーバーが IPv4 だけで
待ち受けているために発生する接続待ちです。`SERVER_HOST` に IP アドレス(`127.0.0.1` など)を
直接書けば消せます。
